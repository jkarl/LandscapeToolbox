...
params:
  crewvar: yes
  dimafile: WRFO_2011-2014_DIMA3.1.mdb #AIM_2013.mdb 
  missingvals: yes
  site: all
  suspectvals: yes
  year: 2012
...
m <- leaflet() %>% addTiles() %>% addMarkers(data.sub$Easting,data.sub$Northing,popup=data.sub$PlotID)
m
```
...
for (i in 1:nrow(plots)){ # loop through the lines
  ...
  plotCode.sum <- rbind(plotCode.sum,data.frame("PlotKey"=thePlot,"PlotID"=thePlotID,"totalSpp"=totalSpp,"sppString"=SppStr,"numUnkn"=unknowns,"numDS"=ds,"numVL"=vl,"numEL"=el,"numCY"=cy,"numNoMatch"=length(noMatch),"noMatches"=noMatchStr,"numSyns"=length(Syns),"Synonyms"=synStr))
...
ggplot(plotCode.melt,aes(x=PlotID,y=value,fill=variable))+geom_bar(position="fill",stat="identity") +
  scale_fill_discrete(name="Plant Codes",labels=c("Valid plant codes","Valid Unknown codes","Codes not in PLANTS db",'"Retired" codes'))+
...
if (nrow(SynMaster)>0){
  ...
  cat("\nAll plants were recorded with current PLANTS Database codes.\n")
}
```
...
if (plotCode.sum$numNoMatch>0) {
  ...
  cat("\nnAll plants recorded with valid PLANTS Database codes.\n")
}
